...
<style>
  ...
  section[data-testid="stSidebar"] input, section[data-testid="stSidebar"] textarea {{
      background-color: #16242B; color: #FFF; }}
...
_POTREE_COLOUR_JS = {
    ...
    "Intensity": 'material.activeAttributeName="intensity";material.gradient=Potree.Gradients.GRAYSCALE;',
}
...
def render_point_cloud(bundle, visible_layers, point_size):
    ...
    with st.spinner("Preparing point cloud…"):
        ...
        r,g,b = _vivid_colors(c["z"].values); c["r"],c["g"],c["b"]=r,g,b
        c["z"]=c["z"].round(1)
        bd = meta["bounds_4326"]
    ...
    st.caption(f"Downsampled · {_fmt_n(nd)} pts from {_fmt_n(meta.get('n_total',0))} raw · elevation colours")
...
def render_dtm(bundle, ve, cs):
    ...
    nr,nc=dtm.shape
    lons=np.linspace(bd["lon_min"],bd["lon_max"],nc); lats=np.linspace(bd["lat_max"],bd["lat_min"],nr)
    ...
    fig=go.Figure(data=[go.Surface(z=zp,x=xm,y=ym,colorscale=cs,cmin=zlo,cmax=zhi,
        colorbar=dict(title=dict(text="Elevation (m)",font=dict(color="#ccc")),
            tickvals=tv,ticktext=[f"{v:.0f}" for v in tv],tickfont=dict(color="#ccc"),thickness=14,len=.7),
        customdata=dtm,hovertemplate="E: %{x:.0f} m<br>N: %{y:.0f} m<br>Elevation: %{customdata:.1f} m<extra></extra>")])
    ...
    st.caption(f"DTM · {nr}×{nc} · vert. exag. {ve:.1f}× · {cs}")
...
def main():
    ...
    bundle=None; vis={}; ps=2; ve=1.0; cs="Turbo"; picked=PLACEHOLDER; potset=None
    ...
    tabs=st.tabs(tabs_l)
    with tabs[0]:
        if potset: render_potree_viewer(bundle,potset)
