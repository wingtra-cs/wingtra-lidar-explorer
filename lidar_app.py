...
def render_header(subtitle="Point cloud · Digital terrain model"):
    ...
    st.markdown(f'<div class="wingtra-header">{brand}<div><div class="wingtra-title">{APP_TITLE}</div>{sub}</div></div>',
...
def _vivid_colors(z_vals):
    v=np.asarray(z_vals,dtype="float64"); fin=v[np.isfinite(v)]
    if fin.size==0: return (np.zeros_like(v,dtype="uint8"),)*3
...
def _extract_contour_segments(cs):
    ...
    try:
        ...
        for i, level in enumerate(cs.levels):
            result[float(level)] = [np.asarray(s) for s in all_segs[i]]
        return result
    ...
    if _paths:
        for i, level in enumerate(cs.levels):
            if i < len(_paths):
                result[float(level)] = [p.vertices for p in _paths[i]
...
def _compute_contour_shapefile(dtm, dtm_meta, interval):
    ...
    levels = np.arange(np.floor(zlo/interval)*interval,
                       np.ceil(zhi/interval)*interval + interval*0.5, interval)
...
#error_overlay{{display:none;position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);
  font-family:-apple-system,sans-serif;text-align:center;max-width:480px;padding:0 20px;z-index:10}}
...
<div id="loading_overlay"><div class="msg">Loading LIDAR point cloud…</div>
  <div class="sub">Streaming tiles from cloud storage</div></div>
<div id="error_overlay"><div class="title">Could not load point cloud</div>
  <div class="detail" id="error_detail">Check console (F12).</div></div>
<script>
(function(){{var _W=window.Worker;var _forceBlob=(window.location.origin==="null");
window.Worker=function(url,opts){{if(!_forceBlob){{try{{return new _W(url,opts)}}catch(e){{}}}}
...
var code=xhr.responseText.replace(/importScripts\\s*\\(\\s*['"]([^'"]+)['"]\\s*\\)/g,
function(m,p){{if(/^https?:\\/\\//.test(p))return m;
try{{return 'importScripts("'+new URL(p,base).href+'")'}}catch(err){{return m}}}});
...
<script>
function showError(msg){{var lo=document.getElementById("loading_overlay"),
eo=document.getElementById("error_overlay"),ed=document.getElementById("error_detail");
if(lo)lo.style.display="none";if(ed)ed.textContent=String(msg);if(eo)eo.style.display="block";}}
if(typeof Potree==="undefined"){{showError("Potree failed to load.");}}
else{{var el=document.getElementById("potree_render_area");
function _awaitSize(){{if(el.clientWidth>0&&el.clientHeight>0)_initViewer();
else requestAnimationFrame(_awaitSize);}}
function _initViewer(){{try{{Potree.scriptPath="{assets_base}";
var viewer=new Potree.Viewer(el);viewer.setEDLEnabled(false);
viewer.setPointBudget({point_budget});viewer.setBackground("black");viewer.setFOV(60);
Potree.loadPointCloud("{potree_url}","{survey_name}",function(e){{
var lo=document.getElementById("loading_overlay");if(lo)lo.style.display="none";
var pc=e.pointcloud,mat=pc.material;mat.activeAttributeName="elevation";
mat.gradient=Potree.Gradients.RAINBOW;mat.size=1;
mat.pointSizeType=Potree.PointSizeType.ADAPTIVE;
viewer.scene.addPointCloud(pc);viewer.fitToScreen();}});
}}catch(e){{showError("Init: "+e);}}}} _awaitSize();}}
</script></body></html>"""
...
def render_potree_viewer(bundle, point_budget):
    ...
    components.html(_potree_html(f"{r2p}/{pk}",f"{r2p}/potree-assets",
        meta.get("name",meta.get("slug","Survey")),point_budget),height=650,scrolling=False)
    co=st.columns(3)
    ...
    st.caption(f"Full {_fmt_n(meta.get('n_total',0))}-point LIDAR · Potree · "
...
# =========================================================================== #
_EMPTY_MAP_STYLE = ("data:application/json,"
...
def render_point_cloud(bundle,visible_layers,point_size):
    meta=bundle["meta"]; import pandas as pd
    frames=[df for n,df in bundle["layers"].items() if visible_layers.get(n,True)]
    if not frames: st.info("No layers selected."); return
    with st.spinner("Preparing point cloud…"):
        c=pd.concat(frames,ignore_index=True).copy()
        r,g,b=_vivid_colors(c["z"].values);c["r"],c["g"],c["b"]=r,g,b;c["z"]=c["z"].round(1)
        bd=meta["bounds_4326"]
        clat=(bd["lat_min"]+bd["lat_max"])/2;clon=(bd["lon_min"]+bd["lon_max"])/2
        zoom=max(10,min(17,round(np.log2(360/(bd["lon_max"]-bd["lon_min"]))-1)))
        layer=pdk.Layer("PointCloudLayer",data=c,get_position=["lon","lat","z"],
              get_color=["r","g","b"],point_size=point_size,pickable=True,
              material={"ambient":.4,"diffuse":.6,"shininess":32,"specularColor":[60,60,60]})
        deck=pdk.Deck(layers=[layer],initial_view_state=pdk.ViewState(
              latitude=clat,longitude=clon,zoom=zoom,pitch=55,bearing=0),
              map_style=_EMPTY_MAP_STYLE,tooltip={"text":"Z: {z} m"},effects=[_LIGHTING])
    st.pydeck_chart(deck,use_container_width=True)
    nd=sum(len(bundle["layers"][n]) for n in visible_layers if visible_layers[n] and n in bundle["layers"])
    co=st.columns(4)
    co[0].metric("Displayed",_fmt_n(nd));co[1].metric("Total",_fmt_n(meta.get("n_total",0)))
    co[2].metric("Z min",f"{meta.get('z_min',0):.1f} m");co[3].metric("Z max",f"{meta.get('z_max',0):.1f} m")
...
def render_dtm(bundle, ve, cs):
    ...
    fm=np.isfinite(dtm)
    if not fm.any(): st.warning("No valid elevations."); return
    zlo,zhi=float(np.min(dtm[fm])),float(np.max(dtm[fm])); relief=zhi-zlo

    bd=dm["bounds_4326"];lat0=(bd["lat_min"]+bd["lat_max"])/2;lon0=(bd["lon_min"]+bd["lon_max"])/2
    mo=111320.*np.cos(np.radians(lat0));ml=111320.
    nr,nc=dtm.shape
    lons=np.linspace(bd["lon_min"],bd["lon_max"],nc)
    lats=np.linspace(bd["lat_max"],bd["lat_min"],nr)
    xm=(lons-lon0)*mo;ym=(lats-lat0)*ml;zp=dtm*ve
    sx=float(xm.max()-xm.min());sy=float(ym.max()-ym.min());sz=relief*ve
    ms=max(sx,sy,sz,1);tv=np.linspace(zlo,zhi,6);bg="rgb(12,12,18)"

    # Contour interval (DTM tab only)
    int_opts=_contour_options(relief)
    labels=["Off"]+[f"{i}m" for i in int_opts]
    contour_choice=st.selectbox("Contour interval",labels,index=0,key="dtm_contour_sel",
        help=f"Elevation contour lines. Survey relief: {relief:.0f}m.")
    contour_int=int(contour_choice.replace("m","")) if contour_choice!="Off" else None
    ...
    if contour_int is not None:
        ...
        contour_cfg=dict(z=dict(show=True,start=c_start,end=c_end,size=contour_int*ve,
            color="rgba(255,255,255,0.5)",width=1,usecolormap=False,
            highlightcolor="rgba(255,255,255,0.8)",project_z=False))

    fig=go.Figure(data=[go.Surface(z=zp,x=xm,y=ym,colorscale=cs,cmin=zlo*ve,cmax=zhi*ve,
        colorbar=dict(title=dict(text="Elevation (m)",font=dict(color="#ccc")),
        ...
        lightposition=dict(x=-500,y=500,z=2000),contours=contour_cfg,hoverinfo="skip")])
    ax=dict(tickfont=dict(color="#888"),gridcolor="#2a2a3a",showbackground=False)
    fig.update_layout(scene=dict(
        xaxis=dict(title=dict(text="Easting (m)",font=dict(color="#aaa")),**ax),
        yaxis=dict(title=dict(text="Northing (m)",font=dict(color="#aaa")),**ax),
        zaxis=dict(title=dict(text="Elevation (m)",font=dict(color="#aaa")),**ax),
        aspectmode="manual",aspectratio=dict(x=sx/ms,y=sy/ms,z=sz/ms),bgcolor=bg),
...
def main():
    ...
    with st.sidebar:
        st.header("Survey");surveys=list_surveys()
        if not surveys:
            st.warning("No surveys in R2.")
            if st.button("⟳ Refresh"): refresh_surveys();st.rerun()
        else:
            ...
            if picked!=PLACEHOLDER:
                ...
                with st.spinner(f"Loading {picked}…"):
                    try: bundle=load_bundle(survey["slug"])
                    except Exception as e: st.error(f"Load error: {e}");bundle=None
                if bundle:
                    meta=bundle["meta"]
                    ...
                    if use_pot:
                        pb=st.select_slider("Point budget",
                            ...
                            help="Max points rendered. Lower = smoother.")
                    else:
                        ps=st.slider("Point size",1,6,2)
                        for n in meta.get("layers",[]): vis[n]=st.checkbox(_layer_label(n),value=True)
                    ...
                    if rlk:
                        try: st.link_button("⬇ Raw LAS",presigned_url(rlk,ttl=900),
                                            use_container_width=True,type="primary")
                        except: st.caption("LAS unavailable")
                    if rdk:
